...
class Juego:
    ...
    def __init__(self, jugadores):
        ...
        shuffle(
            self.jugadores
        )  # determina el orden de los jugadores, 0 es mano en la primera ronda
        self.cantidad_jugadores = len(jugadores)
        assert (
            3 <= self.cantidad_jugadores <= 8
        )  # la cantidad de jugadores debe estar entre 3 y 8
        ...
        self.cartas_sin_triunfo = (
            0  # las rondas sin triunfo se van a jugar con esta cantidad de cartas
        )
        self.lista_rondas = (
            []
        )  # lista con los numeros de cartas repartidas en cada ronda [1,2,3,4, ... , 4,3,2,1]
        self.lista_pies = []

        # just assigned vars
        self._ajustarMazo()  # ajusta el mazo, las cartas sin triunfo y la lista de rondas
        self.cantidad_rondas = len(
            self.lista_rondas
        )  # int que indica la cantidad de rondas totales
    ...
    def _ajustarMazo(self):  # set mazo, set cartas_sin_triunfo, set lista_rondas
        ...
        self.lista_pies = [
            ...
            for _ in range(len(self.lista_rondas) // len(self.jugadores))
        ]
